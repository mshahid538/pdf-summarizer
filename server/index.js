...
if (!process.env.OCTOAI_TOKEN) {
  console.log(
    "No OctoAI API key found. Please rename .env.example to .env and add your OctoAI API key."
  );
  return;
}
...
(async () => {
  ...
  if (choices.length === 0) {
    console.log(
      "No PDFs found in ./files folder. Please add some PDFs and try again."
    );
    return;
  }
  ...
  console.log(
    `${pdfSelected.filename} has ${pdfSelected.chunks.length} pages and ${
      pdfSelected.text.length
    } characteres. This is around ${pdfSelected.text.length / 4} tokens.`
  );
      ...
      messages: [
        {
          role: "system",
          content:
            "You are a tool that Summarizes PDF. This tool is a application script that converts inputs PDF content and outputs a list the main points. Do not communicate with the user directly.",
        },
        {
          role: "assistant",
  ...
  fs.writeFile(
    `./files/${pdfSelected.filename.replace(".pdf", "")}.txt`,
    summary
  );
  console.log(
    `Summary saved to ./files/${pdfSelected.filename.replace(".pdf", "")}.txt`
  );
})();
